...
object ProjectAnalyzer {
    ...
    private fun analyzeOverview(project: Project): ProjectOverview {
        ...
        val sourceFiles = collectSourceFiles(project, projectScope, adapters)
        ...
        val modules = moduleManager.modules.map { module ->
            ...
            val moduleScope = module.moduleScope
            val moduleFiles = collectSourceFiles(project, moduleScope, adapters)
            val extCounts = moduleFiles.groupingBy { it.extension ?: "" }.eachCount()
            ModuleInfo(
                name = module.name,
                type = detectModuleType(module.name),
                dependsOn = deps,
                stats = ModuleStats(
                    classes = moduleFiles.size,
                    kotlinFiles = extCounts["kt"] ?: 0,
                    javaFiles = extCounts["java"] ?: 0
                )
            )
        }
        ...
        val frameworksSummary = detectFrameworksSummary(project, projectScope, adapters)
        ...
        when {
            totalClasses <= 80 -> {
                ...
                classMap = buildClassMap(project, sourceFiles, adapters, includeMembers = totalClasses <= 20)
            }
            totalClasses <= 300 -> {
                level = "class_names"
                classMap = buildClassMap(project, sourceFiles, adapters, includeMembers = false)
            }
            else -> {
                level = if (totalClasses <= 800) "module_digest" else "grouped_digest"
                keyClasses = detectKeyClasses(project, projectScope, sourceFiles, adapters)
            }
        }
    ...
    private fun detectFrameworksSummary(
        ...
        val allFiles = collectSourceFiles(project, scope, adapters)
    ...
    private fun buildClassMap(
        ...
        for (vf in files) {
            ...
            val adapter = adapters.firstOrNull { runCatching { it.canHandle(pf) }.getOrDefault(false) } ?: continue
            val entries = adapter.collectClassEntries(project, pf, includeMembers) ?: continue
            for ((pkg, list) in entries) {
                classMap.getOrPut(pkg) { mutableListOf() }.addAll(list)
            }
        }
        return classMap
    }

    private fun detectKeyClasses(
        project: Project,
        scope: GlobalSearchScope,
        files: Collection<VirtualFile>,
        adapters: List<LanguageAdapter>
        ...
        for (vf in files.take(500)) {
            ...
            val adapter = adapters.firstOrNull { runCatching { it.canHandle(pf) }.getOrDefault(false) } ?: continue
            adapter.collectKeyClassCandidates(project, pf, scope, moduleName)?.let { candidates.addAll(it) }
        }
    ...
    private fun analyzeDependency(project: Project, args: QueryProjectArgs): ProjectOverview {
        ...
        for (adapter in adapters) {
            found = adapter.findClassByFqName(project, scope, targetClass)
            if (found != null) break
        }
        if (found == null) {
        ...
        return buildDependencyResult(project, found, args, adapters)
    }
    ...
    private fun buildDependencyResult(
        ...
        if (maxHops > 1) {
            val hop1Classes = directRefs.flatMap { ref ->
                adapters.mapNotNull { it.getEnclosingClassLike(ref.element) }
            }.distinct()
            ...
            for (hop in 2..maxHops) {
                ...
                for (el in currentHopElements.take(20)) {
                    ...
                    for (ref in refs) {
                        ...
                        for (adapter in adapters) {
                            adapter.getEnclosingClassLike(ref.element)?.let { nextHopElements.add(it) }
                        }
                    }
                }
    ...
    private fun analyzeApiSurface(project: Project, args: QueryProjectArgs): ProjectOverview {
        ...
        val adapters = LanguageAdapter.all(project)
        val files = collectSourceFiles(project, scope, adapters)

        val apiClasses = mutableListOf<ApiClass>()
        for (vf in files) {
            val pf = PsiManager.getInstance(project).findFile(vf) ?: continue
            val adapter = adapters.firstOrNull { runCatching { it.canHandle(pf) }.getOrDefault(false) } ?: continue
            adapter.collectApiClasses(project, pf)?.let { apiClasses.addAll(it) }
        }
